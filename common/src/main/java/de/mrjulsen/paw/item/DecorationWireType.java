...
public class DecorationWireType extends AbstractWireType {

	private static final float HANG_FAC = 0.025f;
	private static final float THICKNESS = Const.PIXEL;	

	public DecorationWireType(ResourceLocation location) {
		super(location);
	}

	@Override
	public int getMaxLength() {
		return ModServerConfig.DECORATION_WIRE_MAX_LENGTH.get();
	}
	...
	public WireBatch buildWire(WireCreationContext context, BlockAndTintGetter level, WireConnectionData customData, WireEdge edge, WireNode nodeA, WireNode nodeB) {
		...
		Wire wire = WireBuilder.createWire("main", context, a, b, CableType.TIGHT, THICKNESS, 0, SegmentControl.createAuto());
