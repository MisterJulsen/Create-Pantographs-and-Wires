 ...
 */
package de.mrjulsen.paw.datafixer.api;
...
public abstract class DataFixesInternals {
    ...
    public static int getModDataVersion(@NotNull CompoundTag compound) {
        return compound.getInt("PAW_DataVersion");
    }
    ...
    public static @NotNull DataFixesInternals get() {
        if (instance == null) {
            ...
            if (latestVanillaSchema == null) {
                PantographsAndWires.LOGGER.warn("[PAW DFU] Failed to initialize! Either someone stopped DFU from initializing,");
                PantographsAndWires.LOGGER.warn("[PAW DFU] or this Minecraft build is hosed.");
                PantographsAndWires.LOGGER.warn("[PAW DFU] Using no-op implementation.");
