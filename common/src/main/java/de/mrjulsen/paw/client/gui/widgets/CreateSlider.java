...
public class CreateSlider extends DLSlider {
    ...
    public CreateSlider(int x, int y, int w, int h, Component text) {
        super(x, y, w, h);
        this.text = text;
        addEventListener(DLGuiCommonEvents.ValueRangeChangedEvent.class, (s, e) -> {
            updateSliderWidth();
            return false;
        });
    ...
    public void renderMainLayer(Graphics graphics, double mouseX, double mouseY, Rectangle renderBounds) {
        ...
        GuiUtils.drawTexture(TEXTURE, graphics, i, i, 0, 0, SLIDER_DOCK_WIDTH, SLIDER_BAR_SIZE);
        GuiUtils.drawTexture(TEXTURE, graphics, width() - SLIDER_DOCK_WIDTH - i, i, 0, 0, SLIDER_DOCK_WIDTH, SLIDER_BAR_SIZE);
        GuiUtils.drawTexture(TEXTURE, graphics, i + SLIDER_DOCK_WIDTH, i, 7, 0, width() - SLIDER_DOCK_WIDTH * 2 - i * 2, SLIDER_BAR_SIZE);
        ...
        GuiUtils.drawTexture(TEXTURE, graphics, sliderX, sliderY, 0, 9, 3, SLIDER_HEIGHT);
        GuiUtils.drawTexture(TEXTURE, graphics, sliderX + 3, sliderY, 4, 9, sliderW - 6, SLIDER_HEIGHT);
        GuiUtils.drawTexture(TEXTURE, graphics, sliderX + sliderW - 3, sliderY, 61, 9, 3, SLIDER_HEIGHT);
        GuiUtils.drawString(graphics, Minecraft.getInstance().font, sliderX + sliderW / 2, height() / 2 - Minecraft.getInstance().font.lineHeight / 2, txt, 0xFF442000, EAlignment.CENTER, false);
    }
    ...
    public void renderFrontLayer(Graphics graphics, double mouseX, double mouseY, Rectangle renderBounds) {        
        if (isSelected() || isDragged()) {
            ...
            GuiUtils.drawString(graphics, font, width() / 2, -font.lineHeight * 2 - 4, text, 0xFFFFFFFF, EAlignment.CENTER, false);
        }
    }
